...
// Function to draw the random quote
...
const buildno = 12; // Example build number
...
var letters = {
  ...
  ["8"]: `sa$90,u,f$2,d,arc$360:1,u,f$-2,arc$360:1`,
  ["9"]: (origin, scale) => DrawBezier(
    ...
    bezierEasing(0.026, [0.984, -1], [1, 1], 0.9561),
    [2, -0.47],
    `u,sa$90,f$2,arc$360:1`
  ),
  ["-"]: `r$90,f$2`,
  [" "]: `f$1`
};

var DrawText = (quote, origin, scale = 1) => {
  const words = quote.split(" ");
  let next = origin;
  for (const word of words) {
    for (const letter of word.split("")) {
      if (typeof letters[letter.toLowerCase()] === "string") {
        next = RunInstructions(letters[letter.toLowerCase()], next, scale);
      } else if (typeof letters[letter.toLowerCase()] === "function") {
        next = letters[letter.toLowerCase()](next, scale);
      } else {
        console.error(`Letter "${letter}" is not defined in the letters object.`);
      }
      next[0] += LetterPadding * scale;
    }
    next[0] += Padding * scale;
  }
};
...
// Function to draw a random quote
const drawRandomQuote = (origin = [0, 0], scale = 1) => {
  const quote = getRandomQuote();
  DrawText(quote, origin, scale);
};

// Usage example
drawRandomQuote([0, 0], 1);
