...
// Generative Quote Generator (quotes are changeable)
...
DrawText("Build No. ", [3, 6], 0.82);
var tubeOrg = [52.15, 5.0];
var tubeScale = 1.35;
for (let i = 0; i <= 6; i++) {
  if (i == 0) {
...
var DrawScrew = (x, y, mSize, lSize) => {
  // an m3x10 screw would be mSize: 3, lSize: 10
  // this can't be scaled because it would mess with the screw size
  DrawBox([x, y], [lSize, mSize]);
  DrawText(`M${mSize}x${lSize}`, [x, y + mSize + 1], 0.75, [2.97, 1]);
  // DrawText("mm", [x+`M${mSize}x${lSize}`.length, 53], 0.53, [2.97, 1]);
}
// Draw the card
// Set page size
setDocDimensions(126, 74);
DrawScrew(87, 20, 5, 35);

// draw the blob
DrawBlobLogo([-203, -141], 0.025);

DrawScrew(87, 50, 3, 10);
DrawScrew(87, 40, 5, 10);
DrawScrew(87, 30, 5, 20);
DrawText("Blot", [12.22, 64], 0.89);
DrawText("Welcome to    lot", [5.72, 51.5], 1.59);
DrawBorpheus([55.3, 57.4], 0.2)
...
Happy Drawing!`, [4.22, 47.0], 0.47, [2.75, 5.1]);
// ============================ Text Engine ============================
// instructions.ts
...
// Draw and display quote with word wrapping
function displayQuote(quote) {
  ...
  for (let word of quote.split(' ')) {
    ...
    for (let char of word) {
      wordWidth += Padding; // Approximate width of each character
    }
    wordWidth += Padding; // Space between words

    if (x + wordWidth > maxLineWidth) {
      x = Padding;
      y += Padding * 2; // Move to the next line
    }
